...
def _serialize_sessions(db: Session, user_id: int) -> List[Dict]:
    items = db.execute(
        select(ChatSession).where(ChatSession.user_id == user_id).order_by(ChatSession.id.asc())
    ).scalars().all()
...
def create_session(chat_id: int, payload: ChatCreateIn | None = None, db: Session = Depends(get_db)):
    user = _get_or_create_user(db, chat_id)
...
def activate_session(chat_id: int, session_id: int, db: Session = Depends(get_db)):
    user = _get_or_create_user(db, chat_id)
    ...
    s = db.query(ChatSession).filter(
        ChatSession.user_id == user.id, ChatSession.id == session_id
    ).first()
    if not s:
        db.rollback()
...
def add_message(chat_id: int, payload: MessageIn, db: Session = Depends(get_db)):
    ...
    # ответ провайдера (экономный или заглушка)
