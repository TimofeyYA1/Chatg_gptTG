...
class Settings(BaseSettings):
    ...
    # поддерживаем оба имени для совместимости
    ...
    OPENAI_MAX_OUTPUT_TOKENS: int = Field(128, validation_alias=AliasChoices("OPENAI_MAX_OUTPUT_TOKENS", "openai_max_output_tokens"))
    OPENAI_CONTEXT_MESSAGES: int = Field(4, validation_alias=AliasChoices("OPENAI_CONTEXT_MESSAGES", "openai_context_messages"))
    OPENAI_TEMPERATURE: float = Field(0.2, validation_alias=AliasChoices("OPENAI_TEMPERATURE", "openai_temperature"))

    model_config = SettingsConfigDict(
        env_file=ENV_FILE,               # <-- абсолютный путь
